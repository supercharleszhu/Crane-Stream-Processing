...
func (w *hothttp) mergeCache(messageId int) {
...
func (w *hothttp) join(data string) {
...
func (w *hothttp) transform(data string) {
	...
	w.ackVal ^= ackVal
	sendMessageSink(ackVal, w.messageId, words[6])
	sendAck(w.messageId, w.ackVal)
}

func (w *hothttp) getMessageId() int {
	return w.messageId
}
func (w *hothttp) setMessageId(id int) {
	w.messageId = id
}
func (w *hothttp) getAckVal() int {
	return w.ackVal
}
func (w *hothttp) setAckVal(ackVal int) {
	w.ackVal = ackVal
}

func (w *hothttp) writeToSDFS() {
